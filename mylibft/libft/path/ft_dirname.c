...
/*   Updated: 2023/11/26 17:29:00 by andeviei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_dirname(char *name)
{
	...
	if (slash_len == -1)
		return (free(trimname), ft_strjoin(1, PATH_CURRENT));
	if (slash_len == 0)
		return (free(trimname), ft_strjoin(1, PATH_ROOT));
	dirname = ft_strsub(trimname, slash_len);
	return (free(trimname), dirname);
}
